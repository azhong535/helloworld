...
def wenzi(selector):
    ...
    file_tieba.write('  招聘职位：'+jobname[0].strip()+'  联系人：'+lxdz[2].strip()+'  公司地址:'+lxdz[4].strip()+"  福利待遇："+fuli.strip()+'  工资：'+gongzi[0].strip()+'  年龄：'+nianling[0].strip()
                     +'  学历:'+xueli[0].strip()+'  招聘人数:'+zprs[5].strip()+'\r\n职位描述：\r\n'+miaosu.strip()+'\r\n')
...
for ye in range(0,60):
    ...
    print(name)
    for dizhi in urlgo:
        file_tieba.write('\r\n'+'第'+str(i)+"章 :"+'\r'+name[i].strip()+qian[i].strip()+'\r\n'+str(dizhi)+'\r\n'+str(time.time()))
        i+=1
        print(i,dizhi)
        jiexi=fenxi(dizhi)
        wenzi(jiexi)
